...
class PVT_MFA(nn.Module):
    """
    PVT-MFA: Pyramid Vision Transformer with Multi-scale Attention Fusion for Parcel Extraction
    PVT-MFA：一种基于金字塔 Transformer 与多尺度注意力融合的遥感地块提取方法
    """
    ...
    def forward(self, x):
        ...
        x0_edge = torch.cat([x0, f_edge], dim=1)  # 64
        x0_edge_strong = self.cbam(x0_edge)
        x0_edge = self.edge_conv(x0_edge_strong)  # 32
