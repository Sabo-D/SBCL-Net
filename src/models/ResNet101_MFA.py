...
class ResNet101Backbone(nn.Module):
    def __init__(self, weights=ResNet101_Weights.IMAGENET1K_V1):
        ...
        # 将ResNet101分为4个阶段 (layer1~4)，输出4个尺度的特征
    ...
    def forward(self, x):
        ...
        return [x1, x2, x3, x4]  # 和PVT类似的多尺度输出
...
class ResNet_MFA(nn.Module):
    def __init__(self, num_classes=1):
        super(ResNet_MFA, self).__init__()
        self.encoder = ResNet101Backbone()
        channels = [256, 512, 1024, 2048]  # ResNet101 output channels

        self.center = ResBlock(channels[-1], channels[-1])  # 2048 -> 2048

        self.decoder4 = DecoderBlock(channels[3], channels[2], channels[2])  # 2048 -> 1024 -> 1024
        self.decoder3 = DecoderBlock(channels[2], channels[1], channels[1])  # 1024 -> 512 -> 512
        self.decoder2 = DecoderBlock(channels[1], channels[0], channels[0])  # 512 -> 256 -> 256
        self.decoder1 = DecoderBlock(channels[0], 32, 32)                     # 256 -> 32 -> 32
        self.skip0 = ResBlock(3, 32)

        self.out_mask = nn.Conv2d(32, num_classes, kernel_size=1)

        self.mkefe = MKEFE()  # 输出通道假设还是 32
        self.edge_conv = ResBlock(64, 32)
        self.beb = BEB()
        self.cbam = CBAM(64)
        self.out_edge = nn.Conv2d(32, num_classes, kernel_size=1)

    def forward(self, x):
        skips = self.encoder(x)  # [B, 256, 512, 1024, 2048]
        x0 = self.skip0(x)
        ...
        # edge stream
        f_edge = self.mkefe(x)
        x0_edge = torch.cat([x0, f_edge], dim=1)
        x0_edge_strong = self.cbam(x0_edge)
        x0_edge = self.edge_conv(x0_edge_strong)
        edge = self.out_edge(x0_edge)
        beb_out = self.beb(x0_edge, d1)
