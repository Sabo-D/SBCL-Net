...
class PVT_MFA_ablate_v0(nn.Module):
    """
    PVT + Decoder + Skip0
    Baseline with only semantic segmentation; shallow features (Skip0) are reserved
    for the final decoder, but no boundary modeling is introduced.
    """
    def __init__(self, num_classes=1, backbone_name='pvt_v2_b2'):
        super(PVT_MFA_ablate_v0, self).__init__()
        ...
        self.skip0 = ResBlock(3, 32)  # 浅层结构特征，暂不与边路交互，仅用于 decoder1

        self.out_mask = nn.Conv2d(32, num_classes, kernel_size=1)  # 生成 mask 预测

    def forward(self, x):
        skips = self.encoder(x)  # PVT主干输出4阶段：[B, C_i, H/4 → H/32]
        x0 = self.skip0(x)       # 原图提取浅层结构信息（仅用于解码）
        ...
        return [mask]  # baseline 只输出 mask


if __name__ == '__main__':
    x = torch.rand(8, 3, 32, 32)
    model = PVT_MFA_ablate_v0()
    y = model(x)
    print(y.shape)