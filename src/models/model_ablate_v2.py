...
class PVT_MFA_ablate_v2(nn.Module):
    """
    V1 + Semantic→Boundary (Skip0 added to boundary)
    Shallow semantic features (Skip0) are fused with handcrafted edge features
    to guide boundary modeling, introducing one-way structure interaction.
    """
    def __init__(self, num_classes=1, backbone_name='pvt_v2_b2'):
        super(PVT_MFA_ablate_v2, self).__init__()
        ...
        # Edge branch with semantic-to-boundary fusion
        self.mkefe = MKEFE()
        self.cbam = CBAM(64)
        self.edge_conv = ResBlock(64, 32)
        self.out_edge = nn.Conv2d(32, num_classes, kernel_size=1)

        # BEB removed in this version (no feedback)
        # self.beb = BEB()  ← removed
    ...
    def forward(self, x):
        ...
        mask = self.out_mask(d1)  # no BEB involvement

        # edge branch (semantic + handcrafted fusion)
        f_edge = self.mkefe(x)
        x0_edge = torch.cat([x0, f_edge], dim=1)  # fusion
        x0_edge = self.cbam(x0_edge)
        x0_edge = self.edge_conv(x0_edge)
